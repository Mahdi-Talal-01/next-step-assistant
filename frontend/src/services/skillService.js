...
export const skillService = {
  ...
  getSkillGrowthTrends: async (skillId, months = 12) => {
    const response = await request.get(
      `/skills/analytics/growth-trends/${skillId}`
    );
    return response.data;
  },
  ...
  // Get job demand per skill
  getJobDemandPerSkill: async () => {
    const response = await request.get("/skills/analytics/job-demand");
    return response.data;
  },

  // Get skill growth rate
  getSkillGrowthRate: async (skillId) => {
    const response = await request.get(
      `/skills/analytics/growth-rate/${skillId}`
    );
    return response.data;
  },

  // Get skill demand trends
  getSkillDemandTrends: async (skillId, months = 12) => {
    const response = await request.get(
      `/skills/analytics/demand-trends/${skillId}`
    );
    return response.data;
  },

  // Get skill analytics
  getSkillAnalytics: async (skillId) => {
    const response = await request.get(`/skills/analytics/skill/${skillId}`);
    return response.data;
  },
};
